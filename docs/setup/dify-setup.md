...
Optional: [share the setup with someone else via DSL export/import](#sharing-this-setup-dsl-exportimport).
...
|---|---|---|
| **OpenAI** | Pay per use (~$0.01 per test session) | LLM: `gpt-5.4-nano` · Embeddings: `text-embedding-3-small` |
...
|---|---|---|
| **Model** | `Gemini 3.5 Flash` | CHAT mode |
...
- **The answer** -- generated by Gemini 3.5 Flash using the retrieved chunks as context
...
## Sharing This Setup (DSL Export/Import)

Dify lets you export an app's structure as a **DSL** (Domain Specific Language) file — a YAML file capturing the node graph, prompts, and model settings. Someone else can import that file into their own Dify workspace to recreate the same Chatflow without rebuilding it node by node.

### Export

1. Open the **Orion HR Assistant** app
2. Click the **"..."** menu (top-right of the canvas, near Publish) → **Export DSL**
3. Dify downloads a `.yml` file containing the START → Knowledge Retrieval → LLM → ANSWER node graph, the system prompt, and the model configuration (provider, model name, temperature, etc.)

### Import

1. In the target Dify workspace: **Studio → Create App → Import DSL File**
2. Upload the `.yml` file — Dify recreates the identical node structure

### What does *not* travel with the DSL file

A successful import shows every node with a green checkmark, which can be misleading — the same false-positive pattern noted earlier for retrieval wiring. Three things must be redone manually before the imported app actually works:

| Missing after import | Why it's missing | What to do |
|---|---|---|
| **Knowledge Base content** | DSL export only captures the app's pipeline config, not the indexed documents or vectors — a known Dify limitation as of 1.9.0 | Create a new Knowledge Base in the target workspace and upload `docs/sample-data/orion-technologies-employee-handbook.pdf`, following [Phase 2](#phase-2--create-a-knowledge-base) |
| **Model provider API keys** | Credentials are workspace-scoped and are never included in a DSL export, by design | Add an LLM + embedding API key under **Settings → Model Provider**, following [Phase 1](#phase-1--add-an-llm-provider) |
| **Knowledge Retrieval node's KB link** | The node references a Knowledge Base by ID, which doesn't exist yet in the new workspace | Open the **Knowledge Retrieval** node → **KNOWLEDGE** section → re-attach the newly created Knowledge Base, following [Phase 3, Step 2](#step-2--connect-your-knowledge-base-to-the-knowledge-retrieval-node) |

> **What to test here:** don't trust a DSL import at face value. After importing, immediately run Test 1 and Test 2 from [Phase 4](#phase-4--test-the-chatbot) (an in-scope question and an out-of-scope question). If the Knowledge Retrieval node is unlinked, the LLM will still generate a fluent-sounding answer from its own training data instead of the handbook — a silent hallucination that the green checkmarks won't reveal.
